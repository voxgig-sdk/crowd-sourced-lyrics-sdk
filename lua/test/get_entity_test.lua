...
describe("GetEntity", function()
  it("should create instance", function()
    ...
    local ent = testsdk:Get(nil)
    assert.is_not_nil(ent)
  end)

  it("should run basic flow", function()
    local setup = get_basic_setup(nil)
    -- Per-op sdk-test-control.json skip.
    local _live = setup.live or false
    for _, _op in ipairs({"load"}) do
      local _should_skip, _reason = runner.is_control_skipped("entityOp", "get." .. _op, _live and "live" or "unit")
    ...
    local get_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.get")))
    local get_ref01_data = nil
    if #get_ref01_data_raw > 0 then
      get_ref01_data = helpers.to_map(get_ref01_data_raw[1][2])
    end

    -- LOAD
    local get_ref01_ent = client:Get(nil)
    local get_ref01_match_dt0 = {
...
function get_basic_setup(extra)
  runner.load_env_local()

  local entity_data_file = _test_dir .. "../../.sdk/test/entity/get/GetTestData.json"
  local f = io.open(entity_data_file, "r")
  if f == nil then
    error("failed to read get test data: " .. entity_data_file)
  end
