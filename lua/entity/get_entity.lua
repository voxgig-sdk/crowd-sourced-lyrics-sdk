...
local GetEntity = {}
GetEntity.__index = GetEntity


function GetEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, GetEntity)
  self._name = "get"
  self._client = client
...
function GetEntity:get_name()
  return self._name
end


function GetEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return GetEntity.new(self._client, opts)
end
...
function GetEntity:mark_deleted()
  self._deleted = true
end


function GetEntity:deleted()
  return true == self._deleted
end


function GetEntity:data_set(args)
  if args ~= nil then
...
function GetEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function GetEntity:match_set(args)
  if args ~= nil then
...
function GetEntity:stream(action, args, callopts)
...
---@param reqmatch GetLoadMatch
---@param ctrl? table
---@return Get
---@return string? err
function GetEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function GetEntity:_run_op(ctx, post_done)
  local utility = self._utility
