...
class TestGetDirect:
    ...
    def test_should_direct_load_get(self):
        setup = _get_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-get", "live" if setup["live"] else "unit")
        if _skip:
