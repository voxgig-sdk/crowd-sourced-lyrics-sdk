# CrowdSourcedLyrics SDK test runner
...
class CrowdSourcedLyricsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CrowdSourcedLyricsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CrowdSourcedLyricsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CrowdSourcedLyricsTestRunner.getenv("CROWD_SOURCED_LYRICS_TEST_LIVE")
        override = CrowdSourcedLyricsTestRunner.getenv("CROWD_SOURCED_LYRICS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CrowdSourcedLyricsTestRunner.getenv(key)
        ...
        explain = CrowdSourcedLyricsTestRunner.getenv("CROWD_SOURCED_LYRICS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CROWD_SOURCED_LYRICS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CrowdSourcedLyricsTestRunner._test_control is not None:
            return CrowdSourcedLyricsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CrowdSourcedLyricsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CrowdSourcedLyricsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CrowdSourcedLyricsTestRunner.load_test_control()
...
def load_env_local():
    CrowdSourcedLyricsTestRunner.load_env_local()


def env_override(m):
    return CrowdSourcedLyricsTestRunner.env_override(m)


def entity_data(v):
    return CrowdSourcedLyricsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CrowdSourcedLyricsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CrowdSourcedLyricsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CrowdSourcedLyricsTestRunner.load_test_control()


def live_client_options():
    return CrowdSourcedLyricsTestRunner.live_client_options()


def live_delay_ms():
    return CrowdSourcedLyricsTestRunner.live_delay_ms()
