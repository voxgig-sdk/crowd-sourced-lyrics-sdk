    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CROWDSOURCEDLYRICS_TEST_GET_ENTID JSON to run live")
        client = setup["client"]
        ...
        get_ref01_data_dt0_loaded, err = get_ref01_ent.load(get_ref01_match_dt0, None)
        assert err is None
...
def _get_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CROWDSOURCEDLYRICS_TEST_GET_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("CROWDSOURCEDLYRICS_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("CROWDSOURCEDLYRICS_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("CROWDSOURCEDLYRICS_TEST_EXPLAIN") == "TRUE",
        "live": _live,
