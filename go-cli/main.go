// Command CrowdSourcedLyrics-cli is a CLI + REPL for the CrowdSourcedLyrics SDK,
...
const prompt = "crowd-sourced-lyrics"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "get"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CROWD_SOURCED_LYRICS_APIKEY (api key), CROWD_SOURCED_LYRICS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CROWD_SOURCED_LYRICS_APIKEY carries the API key and
	// CROWD_SOURCED_LYRICS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CROWD_SOURCED_LYRICS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CROWD_SOURCED_LYRICS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCrowdSourcedLyricsSDK(opts)
