# crowd-sourced-lyrics-cli

boru-driven command-line client **and** interactive REPL for the CrowdSourcedLyrics
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crowd-sourced-lyrics-cli)
make build

# 2. See usage (words, entities, env vars)
./crowd-sourced-lyrics-cli --help

# 3. Provide credentials once, via the environment
export CROWD_SOURCED_LYRICS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./crowd-sourced-lyrics-cli load 1 get            # {id:1} shorthand
./crowd-sourced-lyrics-cli load '{id:1}' get       # explicit match map

# 5. Override the API base URL for a single call
CROWD_SOURCED_LYRICS_BASE=https://api.example.com ./crowd-sourced-lyrics-cli load 1 get

# 6. No arguments -> interactive REPL
./crowd-sourced-lyrics-cli
crowd-sourced-lyrics> load 1 get
crowd-sourced-lyrics> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crowd-sourced-lyrics-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/crowd-sourced-lyrics-cli load 1 get
   ```
...
```sh
./crowd-sourced-lyrics-cli load 1 get          # scalar shorthand for {id:1}
./crowd-sourced-lyrics-cli load '{id:1}' get     # explicit match map
```
...
./crowd-sourced-lyrics-cli load 1 get
```
...
Run with no arguments to open a REPL (prompt `crowd-sourced-lyrics>`). Each line is
evaluated as its own boru expression:

```text
$ ./crowd-sourced-lyrics-cli
crowd-sourced-lyrics> load 1 get
crowd-sourced-lyrics> /help
crowd-sourced-lyrics> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/crowd-sourced-lyrics-cli`. |
...
(`./crowd-sourced-lyrics-cli <expr>`) and interactively (the REPL), and expressions compose the
