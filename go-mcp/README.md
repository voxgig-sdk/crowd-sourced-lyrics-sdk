# crowd-sourced-lyrics-mcp

[MCP](https://modelcontextprotocol.io) server exposing the CrowdSourcedLyrics SDK as
two agent tools — `crowd-sourced-lyrics_list` and `crowd-sourced-lyrics_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/crowd-sourced-lyrics-mcp)
make build

# 2. Provide credentials via the environment
export CROWD_SOURCED_LYRICS_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user crowd-sourced-lyrics \
  -- /absolute/path/to/crowd-sourced-lyrics-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./crowd-sourced-lyrics-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// crowd-sourced-lyrics_list: first page of records
{ "entity": "get" }
{ "entity": "get", "query": { } }

// crowd-sourced-lyrics_load: one record by id
{ "entity": "get", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/crowd-sourced-lyrics-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user crowd-sourced-lyrics \
     -- "$PWD"/dist/*/crowd-sourced-lyrics-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `crowd-sourced-lyrics_list` and `crowd-sourced-lyrics_load` tools now appear
   in new sessions. Ask the agent to *"list get using crowd-sourced-lyrics"*
   and it calls `crowd-sourced-lyrics_list` with `{"entity":"get"}`.
...
```sh
export CROWD_SOURCED_LYRICS_APIKEY=sk_live_xxx            # API key
export CROWD_SOURCED_LYRICS_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `crowd-sourced-lyrics_list` tool
...
### Call the `crowd-sourced-lyrics_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `crowd-sourced-lyrics_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `crowd-sourced-lyrics_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CROWD_SOURCED_LYRICS_APIKEY` | API key sent with every request. |
| `CROWD_SOURCED_LYRICS_BASE` | Optional override of the API base URL. |
...
```sh
./crowd-sourced-lyrics-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"crowd-sourced-lyrics_load","arguments":{"entity":"get","query":{"id":1}}}}'
```
