...
module CrowdSourcedLyricsUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CrowdSourcedLyricsResponse.new({})
    ...
    if fetch_err
      ...
      response = CrowdSourcedLyricsResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CrowdSourcedLyricsResponse.new(fetched)
    else
