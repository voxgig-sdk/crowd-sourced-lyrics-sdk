...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = CrowdSourcedLyricsOmni.make_runner(TEST_JSON_FILE, CrowdSourcedLyricsSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    CrowdSourcedLyricsError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : CrowdSourcedLyricsOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = CrowdSourcedLyricsOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
