# CrowdSourcedLyrics Ruby SDK



The Ruby SDK for the CrowdSourcedLyrics API — an entity-oriented client using idiomatic Ruby conventions.
...
```ruby
client = CrowdSourcedLyricsSDK.test(testopts, sdkopts)
```
...
raise a `CrowdSourcedLyricsError` (a `StandardError` subclass), so wrap
...
```
rb/
├── CrowdSourcedLyrics_sdk.rb       -- Main SDK module
...
The main module (`CrowdSourcedLyrics_sdk`) exports the SDK class
