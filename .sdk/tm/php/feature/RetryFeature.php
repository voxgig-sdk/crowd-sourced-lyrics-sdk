...
// CrowdSourcedLyrics SDK retry feature
...
class CrowdSourcedLyricsRetryFeature extends CrowdSourcedLyricsBaseFeature
{
    ...
    public function init(CrowdSourcedLyricsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CrowdSourcedLyricsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CrowdSourcedLyricsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CrowdSourcedLyricsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
