...
module CrowdSourcedLyricsOmni
  ...
  class ObjView < ::Hash
    ...
    def view_wrap(name)
      ...
      return val if CrowdSourcedLyricsOmni.plainval?(val) || seen.key?(val.object_id)
    ...
    def view_attrname(key)
      ...
      snake = CrowdSourcedLyricsOmni.underscore(name)
    ...
    def keys
      out = view_names.map { |n| CrowdSourcedLyricsOmni.camelize(n) }
    ...
    def []=(key, val)
      ...
      t = @view_target
      name = view_attrname(key) || CrowdSourcedLyricsOmni.underscore(key.to_s)
      setter = "#{name}="
  ...
  def enrich(ctxmap, ctx)
    ...
    ctx.spec = CrowdSourcedLyricsSpec.new(spec_map) if spec_map.is_a?(::Hash)
    ...
    if res_map.is_a?(::Hash)
      ...
      if err_map.is_a?(::Hash) && err_map['message'].is_a?(::String) &&
        ...
        ctx.result.err = CrowdSourcedLyricsError.new('', err_map['message'])
      end
    end
  ...
  def sdkhooks(sdk)
    ...
    errify = ->(err) { CrowdSourcedLyricsOmni.errify(err) }
