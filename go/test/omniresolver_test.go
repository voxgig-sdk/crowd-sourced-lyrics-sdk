...
import (
	...
	omni "github.com/voxgig-sdk/crowd-sourced-lyrics-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CrowdSourcedLyricsError); is {
				out := map[string]any{
					"name":    "CrowdSourcedLyricsError",
...
func omniCtx(arg any, client *sdk.CrowdSourcedLyricsSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CrowdSourcedLyricsSDK); is {
			client = live
