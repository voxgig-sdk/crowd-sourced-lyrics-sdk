...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CROWDSOURCEDLYRICS_TEST_LIVE") == "TRUE" ||
		os.Getenv("CROWDSOURCEDLYRICS_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CROWDSOURCEDLYRICS_TEST_EXPLAIN"); explain != "" {
