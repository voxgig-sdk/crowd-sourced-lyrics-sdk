...
class GetEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        ...
        $ent = $testsdk->Get(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = get_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["load"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "get." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        $get_ref01_data = null;
        if (count($get_ref01_data_raw) > 0) {
            $get_ref01_data = Helpers::to_map($get_ref01_data_raw[0][1]);
        }

        // LOAD
        $get_ref01_ent = $client->Get(null);
        $get_ref01_match_dt0 = [
            "id" => $get_ref01_data["id"],
        ];
        [$get_ref01_data_dt0_loaded, $err] = $get_ref01_ent->load($get_ref01_match_dt0, null);
        $this->assertNull($err);
...
function get_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/get/GetTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["get01", "get02", "get03"] as $k) {
