...
class GetDirectTest extends TestCase
{
    public function test_direct_load_get(): void
    {
        $setup = get_direct_setup(["id" => "direct01"]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-load-get", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
...
function get_direct_setup($mockres)
{
    ...
    if ($live) {
        ...
        $client = new CrowdSourcedLyricsSDK($merged_opts);
        return [
            "client" => $client,
            "calls" => $calls,
            "live" => true,
