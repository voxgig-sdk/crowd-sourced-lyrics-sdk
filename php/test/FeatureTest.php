...
// CrowdSourcedLyrics SDK feature test
//
...
class FtCtrl extends CrowdSourcedLyricsControl
{
...
class FtClient
{
    ...
    private CrowdSourcedLyricsUtility $utility;

    public function __construct(CrowdSourcedLyricsUtility $utility, array $options)
    {
    ...
    public function get_utility(): CrowdSourcedLyricsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CrowdSourcedLyricsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CrowdSourcedLyricsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CrowdSourcedLyricsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CrowdSourcedLyricsConfig::make_feature($name);
    ...
    private static function buildUrl(CrowdSourcedLyricsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CrowdSourcedLyricsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CrowdSourcedLyricsResponse($fetched);
        }
    ...
    private function fail(CrowdSourcedLyricsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CrowdSourcedLyricsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
