...
//    only - a typed CrowdSourcedLyricsContext there would make every ctx
...
final class CrowdSourcedLyricsOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new CrowdSourcedLyricsContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new CrowdSourcedLyricsSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new CrowdSourcedLyricsResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new CrowdSourcedLyricsError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new CrowdSourcedLyricsResponse($clean['response']);
    ...
    public static function err_from_map($m): ?CrowdSourcedLyricsError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new CrowdSourcedLyricsError(is_string($code) ? $code : '', $msg);
    }
