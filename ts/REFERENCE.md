# CrowdSourcedLyrics TypeScript SDK Reference

Complete API reference for the CrowdSourcedLyrics TypeScript SDK.
...
```ts
new CrowdSourcedLyricsSDK(options?: object)
```
...
#### `CrowdSourcedLyricsSDK.test(testopts?, sdkopts?)`
...
**Returns:** `CrowdSourcedLyricsSDK` instance in test mode.
...
Create a new `Get` entity instance.
...
Alias for `CrowdSourcedLyricsSDK.test()`.
...
```ts
const result = await client.Get().load()
```
...
Return the parent `CrowdSourcedLyricsSDK` instance.
