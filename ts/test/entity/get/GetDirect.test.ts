...
describe('GetDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CrowdSourcedLyricsSDK({
  ...
  test('direct-load-get', async (t: any) => {
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CROWD_SOURCED_LYRICS_TEST_GET_ENTID']
