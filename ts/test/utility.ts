...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.CROWDSOURCEDLYRICS_TEST_LIVE ||
    'TRUE' === process.env.CROWDSOURCEDLYRICS_TEST_OVERRIDE
  ) {
  ...
  m.CROWDSOURCEDLYRICS_TEST_EXPLAIN = process.env.CROWDSOURCEDLYRICS_TEST_EXPLAIN || m.CROWDSOURCEDLYRICS_TEST_EXPLAIN
