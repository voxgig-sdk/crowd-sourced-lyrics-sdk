...
//     (\`new CrowdSourcedLyricsSDK(...)\` / \`CrowdSourcedLyricsSDK.test(...)\` become
//     \`CrowdSourcedLyricsSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CrowdSourcedLyricsSDK'
...
const TEST_SEED = {"entity":{"get":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CrowdSourcedLyricsSDK(...)\` and any
// \`CrowdSourcedLyricsSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CrowdSourcedLyricsSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CrowdSourcedLyricsSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
